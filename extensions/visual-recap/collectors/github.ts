...
export async function collectPr(
  ...
  if (target.kind !== "pr") {
    throw new Error(`collectPr called with non-pr target: ${(target as { kind: string }).kind}`);
  }
  ...
  if (ghPath) {
    const evidence = await collectViaGh(ghPath.prNumber, ghPath.repoSlug, options);
...
async function resolveGhRepo(
  ...
  if (prNumber) {
    const remote = await tryExec("gh", ["repo", "view", "--json", "nameWithOwner"], options);
    if (remote && remote.exitCode === 0) {
      try {
        ...
        if (parsed.nameWithOwner) {
          const test = await tryExec(
            "gh",
            ["pr", "view", String(prNumber), "--repo", parsed.nameWithOwner, "--json", "title"],
            options,
          );
          if (test && test.exitCode === 0) {
...
async function loadPrCommits(
  ...
  try {
    ...
    return parsed.commits.map((c: any) => ({
      ...
      subject: String(c.messageHeadline ?? c.messageHead ?? c.message ?? "").split("\n")[0] ?? "",
      author: String(c.authors?.[0]?.login ?? c.author?.login ?? c.author ?? "unknown"),
...
async function collectViaRest(
  ...
  const headers = ["Accept: application/vnd.github+json", "User-Agent: pi-visual-recap"];
  const meta = await tryExec(
    "curl",
...
function parsePrTarget(raw: string): { prNumber: number | null; repoSlug: string | null; fromUrl: boolean } {
  ...
  if (/^\d+$/.test(trimmed)) {
    return { prNumber: Number.parseInt(trimmed, 10), repoSlug: null, fromUrl: false };
  }
  const ownerRepo = trimmed.match(/^([^/\s]+)\/([^/\s]+)$/);
  if (ownerRepo) {
    return { prNumber: null, repoSlug: `${ownerRepo[1]}/${ownerRepo[2]}`, fromUrl: false };
  }
...
function parseUnifiedDiff(diffText: string): ChangedFile[] {
  ...
  for (const line of lines) {
    if (line.startsWith("diff --git ")) {
      ...
      current = {
        path,
        oldPath: match?.[1] && match[2] && match[1] !== match[2] ? match[1] : undefined,
        status: "modified",
        additions: 0,
        deletions: 0,
        binary: false,
