...
import { evidenceFromGit, evidenceFromPr, evidenceFromSession } from "./analysis/normalize.ts";
import { buildSystemPrompt, buildUserPrompt, coerceRecapDocument } from "./analysis/prompts.ts";
...
import type { RecapDocument, RecapEvidence, RecapTarget, VisualRecapOptions } from "./schemas.ts";
...
export async function generateRecap(
  ...
  const target = parseTarget(rawArgs, { defaultBranch: detectDefaultBranch(ctx) });
  ...
  const messages: Message[] = [
    { role: "user", content: [{ type: "text", text: buildSystemPrompt() }], timestamp: Date.now() },
    {
      role: "user",
      content: [{ type: "text", text: buildUserPrompt(evidence, evidence.targetLabel) }],
      timestamp: Date.now(),
    },
  ];
  ...
  if (format === "md" || format === "all") files["recap.md"] = renderMarkdown(document);
  if (format === "json" || format === "all") files["recap.json"] = renderJson(document);
  if (format === "mdx" || format === "all") files["recap.mdx"] = renderMdx(document);
  if (format === "html" || format === "all") files["index.html"] = renderHtml(document);
  ...
  return { document, artifactDir: writeResult.dir, written: writeResult.written };
}
...
async function collectEvidence(
  ...
  switch (target.kind) {
    ...
    case "session": {
      const evidence = await collectSession({ ctx, session: target.session });
...
function isEvidenceEmpty(evidence: RecapEvidence): boolean {
  if (evidence.source === "pi-session") {
    ...
    return s.branchLength === 0 || (s.totalMessages === 0 && s.turns.length === 0);
  }
...
function targetLabelFor(target: RecapTarget): string {
  switch (target.kind) {
    ...
    case "session":
      return target.session === "current" ? "current Pi session" : `session ${target.session}`;
  }
}
...
async function readProjectConfig(ctx: ExtensionCommandContext): Promise<VisualRecapConfig | undefined> {
...
export function registerVisualRecapCommand(pi: ExtensionAPI): void {
  pi.registerCommand(COMMAND_NAME, {
    description: "Create a visual recap for a commit, range, branch, PR, or Pi session",
    handler: async (args, ctx) => {
      try {
        ...
        if (ctx.hasUI) ctx.ui.notify(`Visual recap failed: ${message}`, "error");
      }
    },
  });
}
    ...
    parameters: Type.Object({
      ...
      format: Type.Optional(
        StringEnum(["md", "json", "mdx", "html", "all"] as const, {
          description: "Output format. Defaults to all (md + json + mdx + html).",
        }),
      ),
      outputDir: Type.Optional(
        Type.String({ description: "Output directory relative to cwd. Defaults to .visual-recaps" }),
      ),
      maxDiffBytes: Type.Optional(
        Type.Number({ description: "Cap on diff text sent to the model (bytes). Defaults to 750_000." }),
      ),
    }),
    async execute(_toolCallId, params, _signal, onUpdate, ctx) {
      try {
        const result = await generateRecap(params.target ?? "", params, ctx as ExtensionCommandContext);
        ...
        if (onUpdate) onUpdate({ content: [{ type: "text", text: summary }], details });
        return {
